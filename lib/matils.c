...
/*
 * multiply matrices of arbitrary sizes
*/
...
double *weightedSum(double **arr, double *wsArr ,int width, int length)
{
  double *weightArr = malloc(width * sizeof(double));

  for (int i = 0; i < width; i++)
  {
    weightArr[i] = randfloat(0, 1);
  }

  multiplyMatrices(arr, weightArr, wsArr, length, width, width);
  return wsArr;
}
